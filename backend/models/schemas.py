...
class CompetitorAnalysis(BaseModel):
    ...
    design_score: int = Field(5, ge=0, le=10, description="Оценка визуального стиля и дизайна (0-10)")
    animation_potential: str = Field("", description="Потенциал для анимации и интерактивности")
...
class CompetitorCollectionResponse(BaseModel):
    """Ответ на автоматический сбор данных конкурентов"""
    success: bool
    competitors: List[ParsedContent] = Field(default_factory=list, description="Список данных конкурентов")
    total_processed: int = Field(0, description="Общее количество обработанных сайтов")
