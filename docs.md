...
| `OPENAI_MODEL` | Модель для текста | `google/gemini-2.5-flash-lite-preview-09-2025` |
| `OPENAI_VISION_MODEL` | Модель для изображений | `google/gemini-2.5-flash-lite-preview-09-2025` |
| `API_HOST` | Хост сервера | `0.0.0.0` |
| `API_PORT` | Порт сервера | `8000` |

### OpenRouter

Проект использует [OpenRouter](https://openrouter.ai/) — OpenAI-совместимый API для доступа к различным моделям ИИ, включая Google Gemini.
- Доступ к множеству моделей
- Единая точка входа для разных провайдеров
